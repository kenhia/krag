...
class LLMClient:
    ...
    def generate_stream(
        ...
        """Generate answer tokens from chat messages, yielding each token.

        Uses ``create_chat_completion(stream=True)`` which returns an
        iterator of partial completions.  Each yielded string is a token
        delta (one or a few characters).
        ...
        Yields:
            Token delta strings as they are generated by the LLM.
        """
        if not messages:
            yield self._generate_fallback([])
            return
        ...
        if self.model is None:
            logger.debug("No model loaded, using fallback response for streaming")
            yield self._generate_fallback(messages)
            return

        logger.debug(
            "Streaming with temperature=%.2f, max_tokens=%d, top_p=%.2f, repeat_penalty=%.2f",
            temp,
            max_tok,
            t_p,
            r_p,
        ...
        try:
            ...
            logger.error("LLM streaming generation failed: %s", e)
            raise
