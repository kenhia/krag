...
<script lang="ts">
	...
	import { transcript, addEntry, updateEntry } from "$lib/state/transcript.svelte";
	...
	import type { QueryRequest, RetrieveRequest, QueryStreamEvent, SourceChunk } from "$lib/types";
	...
	async function streamQuery(entryId: string, req: QueryRequest, startTime: number): Promise<void> {
		...
		await streamQuerySSE(baseUrl, req, (event: QueryStreamEvent) => {
			switch (event.type) {
				case "query:sources":
	...
	<div class="query-input-area">
		...
		<div class="query-actions">
			<label class="retrieve-toggle">
				<input type="checkbox" bind:checked={retrieveOnly} disabled={!isConnected} />
				<span class="toggle-label">Retrieve Only</span>
			</label>
			...
			<div class="send-area">
				...
				{/if}
				<Button
					label={retrieveOnly ? "Retrieve" : "Send"}
					variant="primary"
					disabled={!canSend}
					{loading}
					onclick={handleSend}
...
<style>
	...
	.retrieve-toggle input[type="checkbox"] {
		accent-color: var(--accent, #89b4fa);
		cursor: pointer;
	}

	.retrieve-toggle input:disabled {
		cursor: not-allowed;
	}
